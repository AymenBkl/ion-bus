...
export class OrderDetailPage implements OnInit {
  ...
  placeOrder() {
    ...
    this.api.updateOrder(this.id, param).then((data) => {
      ...
      if (this.ets) {
        const msg = this.util.translate('Your Order is ') + value + this.util.translate(' By ') + this.restName + this.util.translate('. Order nymber is ') + this.orderid + ' and ' + this.util.translate(' estimated delivery time is ') +this.ets;
        alert(msg)
        ...
        const msg = this.util.translate('Your Order is ') + value + this.util.translate(' By ') + this.restName + this.util.translate('. Order nymber is ') + this.orderid + ' and ' + this.util.translate(' estimated delivery time is ') + 'N/A'
        this.api.sendNotification(msg, 'Order ' + value, this.token).subscribe((data) => {
          console.log(data);
          this.util.hide();
          this.navCtrl.back();
  ...
  async EstTime(){
    ...
    const alert = await this.alert.create({
      header: 'Enter estimated delivery time',
      inputs: [
        {
          name: 'radio1',
          type: 'radio',
          ...
          handler: (data) => {
            ...
            return new Promise<any>(async (resolve, reject) => {
              this.adb
                .collection("orders")
                .doc(this.id)
                .update({ deliveryTime: data })
                .then(async (order: any) => {

                  this.changeStatus(acc)
                  resolve(order);
  ...
  printOrder() {
    ...
    const content = '<div style="padding: 20px;display: flex;flex-direction: column;"> <img src="assets/imgs/logo.jpg" style="text-align: center; height: 100px;width: 100px;" alt=""> <h2 style="text-align: center;">Foodies Order Summary</h2> <p style="float: left;margin:0px;"><b>' +
