...
export class VenueProfilePage implements OnInit {
  ...
  createVenue() {
    ...
    const param = {
      ...
      allergenInfoImage: this.allergenInfoImage,
    ...
    this.api
      ...
      .then(
        (data) => {
          ...
          console.log(data);
          this.util.showToast(
            this.util.translate("Restaurant updated successfully"),
            "success",
            "bottom"
          );
  ...
  submit() {
    ...
    if (this.minimumOrder) {
      if (isNaN(this.minimumOrder)) {
        this.util.errorToast(
          this.util.translate("Please enter valid Minimum Order Value")
        );
      }
    }
    ...
    if (this.haveData) {
      console.log("update");
      this.util.errorToast("Change info");
      this.updateVenue();
    } else {
      console.log("create");
      this.createVenue();
  ...
  ionViewWillEnter() {
    console.log(
      ...
      localStorage.getItem("newAddress")
    );
    this.util.show();
    this.api
      ...
      .then((data) => {
        ...
        if (data) {
          ...
          if (
            localStorage.getItem("newAddress") &&
            localStorage.getItem("newAddress") != null
          ) {
            this.address = localStorage.getItem("newAddress");
          }
          if (
            localStorage.getItem("newLng") &&
            localStorage.getItem("newLng") != null
          ) {
            this.longitude = localStorage.getItem("newLng");
          }
          if (
            localStorage.getItem("newLat") &&
            localStorage.getItem("newLat") != null
          ) {
          ...
          .then(
            (snapshot) => {
              ...
              snapshot.ref.getDownloadURL().then((url) => {
                ...
                if (!num) {
                  this.coverImage = url;
                } else if (num === "allergen") {
                  this.allergenInfoImage = url;
                } else if (num === 1 || num === "1") {
                  this.image1 = url;
                } else if (num === 2 || num === "2") {
                  this.image2 = url;
